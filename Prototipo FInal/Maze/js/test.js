...
openChest() {
    ...
    console.log("Abriendo cofre, playerId:", playerId);

    const mostrarPopup = (card, secret) => {
        console.log("Mostrando popup con carta:", card);
    ...
    const obtenerSecretoYMostrar = (card) => {
        if (Math.random() < 0.25 && playerId) {
            fetch('http://localhost:3000/api/secrets')
                .then(res => res.json())
                .then(data => {
                    console.log("Secretos recibidos:", data);
    ...
    fetch(`http://localhost:3000/api/player/${playerId}/cards/available`)
        .then(res => res.json())
        .then(data => {
            console.log("Cartas disponibles:", data);
