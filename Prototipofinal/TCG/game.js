...
class Draw {
  ...
  drawPlayerHand(state, selectedCard) {
    ...
    for (let i = 0; i < state.player.hand.length; i++) {
      ...
      if (card.spriteImg) {
        ctx.drawImage(card.spriteImg, x + 10, 655, 60, 60);
      }
 
      ctx.fillStyle = '#ffffff';
      const name = card.name.length > 10 ? card.name.substring(0, 9) + '...' : card.name;
      ctx.fillText(name,              x + 5, 670);
      ctx.fillText('Cost: ' + card.cost, x + 5, 725);
      ctx.fillText('ATK: ' + card.atk,  x + 5, 740);
      ctx.fillText('HP: ' + card.hp,    x + 5, 755);
    }
  ...
  drawOppField(state) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (card) {
        ...
        if (card.spriteImg) {
          ctx.drawImage(card.spriteImg, x + 5, 105, 60, 50);
        }
        
        ctx.fillStyle = '#ffffff';
        const name = card.name.length > 8 ? card.name.substring(0, 7) + '.' : card.name;
        ctx.fillText(name,                x + 5, 120);
        ctx.fillText('ATK:' + card.atk,   x + 5, 165);
        ctx.fillText('HP:'  + card.currentHP, x + 5, 180);
      } else {
    ...
    if (opponent.activeCard) {
      ...
      if (card.spriteImg) {
        ctx.drawImage(card.spriteImg, 560, 110, 80, 70);
      }
      ...
      ctx.fillText('ATK: ' + card.atk,                    555, 195);
      ctx.fillText('HP: ' + card.currentHP + '/' + card.hp, 555, 210);
    } else {
      ctx.strokeStyle = '#ff0000';
      ctx.lineWidth   = 3;
      ctx.strokeRect(550, 100, 100, 120);
  ...
  drawPlayerField(state, selectedCard) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (card) {
        ...
        if (card.spriteImg) {
          ctx.drawImage(card.spriteImg, x + 5, 485, 60, 50);
        }
        
        ctx.fillStyle = '#ffffff';
        const name = card.name.length > 8 ? card.name.substring(0, 7) + '.' : card.name;
        ctx.fillText(name,                x + 5, 500);
        ctx.fillText('ATK:' + card.atk,   x + 5, 545);
        ctx.fillText('HP:'  + card.currentHP, x + 5, 560);
      } else {
    ...
    if (player.activeCard) {
      ...
      if (card.spriteImg) {
        ctx.drawImage(card.spriteImg, 560, 490, 80, 70);
      }
      ...
      ctx.fillText('ATK: ' + card.atk,                    555, 575);
      ctx.fillText('HP: ' + card.currentHP + '/' + card.hp, 555, 590);
    } else {
