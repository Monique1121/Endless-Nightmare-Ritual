...
app.get('/api/player/:playerId/inventory', async (req, res) => {
    ...
    try {
        ...
        // Obtener secretos descubiertos
        const [secrets] = await pool.query(
            `SELECT ps.Secret_id, s.Secret_name, s.Content
...
app.post('/api/player/:playerId/deck/initialize', async (req, res) => {
    ...
    try {
        ...
        res.json({ 
            success: true, 
            message: 'Deck inicializado',
            cards: randomCards.map(c => c.Card_id)
        });
        ...
        res.status(500).json({ 
            success: false, 
            message: 'Error al inicializar deck' 
        });
    }
});

// Resetear jugador (Nueva Partida)
app.post('/api/player/:playerId/reset', async (req, res) => {
    const { playerId } = req.params;

    try {
        // Eliminar todas las cartas del deck
        await pool.query(`DELETE FROM Deck WHERE Player_id = ?`, [playerId]);
        
        // Eliminar todos los secretos descubiertos
        await pool.query(`DELETE FROM Player_Secrets WHERE Player_id = ?`, [playerId]);
        
        // Resetear estadísticas del jugador
        await pool.query(
            `UPDATE Player 
             SET Level = 0,
                 Blood_current = Blood_max,
                 School_unlocked = 1,
                 Hospital_unlocked = 0,
                 Laboratory_unlocked = 0,
                 Secrets_discovered = 0
             WHERE Player_id = ?`,
            [playerId]
        );

        // Inicializar deck con 5 cartas aleatorias (comunes)
        const [randomCards] = await pool.query(
            `SELECT Card_id FROM Cards WHERE Card_id <= 15 ORDER BY RAND() LIMIT 5`
        );
        ...
        res.json({ 
            success: true, 
            message: 'Jugador reseteado exitosamente',
            initialCards: randomCards.map(c => c.Card_id)
        });
        ...
        res.status(500).json({ 
            success: false, 
            message: 'Error al resetear jugador' 
        });
    }
});
