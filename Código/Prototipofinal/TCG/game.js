...
class Draw {
  ...
  getCombatLogAppearance(logMsg) {
    if (logMsg.includes('===')) {
      return {
        fillStyle: TCG_THEME.colors.brass,
        font: this.font(15, TCG_THEME.fonts.mono, 700)
      };
    }

    if (logMsg.includes('---') || logMsg.includes('TURNO')) {
      return {
        fillStyle: TCG_THEME.colors.brass,
        font: this.font(14, TCG_THEME.fonts.mono, 700)
      };
    }

    if (logMsg.includes('[JUGADOR]')) {
      return {
        fillStyle: TCG_THEME.colors.life,
        font: this.font(14, TCG_THEME.fonts.mono, 600)
      };
    }

    if (logMsg.includes('[ENEMIGO]')) {
      return {
        fillStyle: TCG_THEME.colors.blood,
        font: this.font(14, TCG_THEME.fonts.mono, 600)
      };
    }

    if (logMsg.includes('ATAQUE') || logMsg.includes('Ataque')) {
      return {
        fillStyle: TCG_THEME.colors.brass,
        font: this.font(14, TCG_THEME.fonts.mono, 700)
      };
    ...
    if (logMsg.includes('MUERTA') || logMsg.includes('Destruyo')) {
      return {
        fillStyle: TCG_THEME.colors.ritualBright,
        font: this.font(14, TCG_THEME.fonts.mono, 700)
      };
    }

    if (logMsg.includes('VICTORIA') || logMsg.includes('DERROTA')) {
      return {
        fillStyle: TCG_THEME.colors.brass,
        font: this.font(16, TCG_THEME.fonts.mono, 700)
      };
    }
  ...
  wrapCombatLogMessage(text, maxWidth, font) {
    ...
    for (const token of tokens) {
      const segments = ctx.measureText(token).width <= maxWidth
        ? [token]
        : this.splitCombatLogToken(token, maxWidth);
  ...
  getVisibleCombatLogEntries(logs, maxWidth, lineCapacity, maxLogs) {
    ...
    for (let index = recentLogs.length - 1; index >= 0; index--) {
      const message = recentLogs[index];
      const appearance = this.getCombatLogAppearance(message);
      const wrappedLines = this.wrapCombatLogMessage(message, maxWidth, appearance.font);

      if (usedLines + wrappedLines.length > lineCapacity) {
        break;
      }

      visibleEntries.unshift({ appearance, wrappedLines });
      usedLines += wrappedLines.length;
    }

    if (visibleEntries.length === 0 && recentLogs.length > 0) {
      const message = recentLogs[recentLogs.length - 1];
      const appearance = this.getCombatLogAppearance(message);
      const wrappedLines = this.wrapCombatLogMessage(message, maxWidth, appearance.font);
      visibleEntries.push({
        appearance,
        wrappedLines: wrappedLines.slice(-Math.max(1, lineCapacity))
      });
    }

    return {
      visibleEntries,
      hiddenMessages: Math.max(0, logs.length - visibleEntries.length)
    };
  }
  ...
  drawCombatLog(logs) {
    ...
    const bodyHeight = height - headerHeight - (padding * 2) - 8;
    const lineCapacityWithoutFooter = Math.max(1, Math.floor(bodyHeight / lineHeight));
    let { visibleEntries, hiddenMessages } = this.getVisibleCombatLogEntries(
      logs,
      textWidth,
      lineCapacityWithoutFooter,
      maxLogs
    );

    if (hiddenMessages > 0) {
      const lineCapacityWithFooter = Math.max(1, Math.floor((bodyHeight - footerHeight) / lineHeight));
      ({ visibleEntries, hiddenMessages } = this.getVisibleCombatLogEntries(
        logs,
        textWidth,
        lineCapacityWithFooter,
        maxLogs
      ));
    }

    const footerVisible = hiddenMessages > 0;
    const startY = y + headerHeight + padding + 8;
    const clipTop = y + headerHeight + 4;
    const clipBottom = y + height - (footerVisible ? footerHeight : 0) - 6;
    let logY = startY;

    ctx.save();
    ctx.beginPath();
    ctx.rect(x + 6, clipTop, width - 12, clipBottom - clipTop);
    ctx.clip();

    visibleEntries.forEach(({ appearance, wrappedLines }) => {
      ctx.fillStyle = appearance.fillStyle;
      ctx.font = appearance.font;

      wrappedLines.forEach((line) => {
        ctx.fillText(line, x + padding, logY);
        logY += lineHeight;
      });
    });
    ...
    if (footerVisible) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(`+${hiddenMessages} mensajes anteriores`, x + width / 2, y + height - 15);
      ctx.textAlign = 'left';
    }
  }
}
