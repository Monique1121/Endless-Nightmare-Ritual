...
class Game {
    ...
    update(deltaTime) {
        ...
        if (!this.inventoryOpen) {
            ...
            // Verificar salida del laboratorio
...
function handleKeyDown(event) {
    ...
    // Tecla S para cambiar a secretos
    if ((key === "s" || key === "S") && game.inventoryOpen && !game.viewingSecrets) {
        game.toggleSecretsView();
        return;
    }
    
    // Tecla C para cambiar a cartas
    if ((key === "c" || key === "C") && game.inventoryOpen && game.viewingSecrets) {
        game.toggleSecretsView();
        return;
    }
